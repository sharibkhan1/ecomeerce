...
const OrderItemListPage: React.FC = () => {
    ...
    items.forEach(item => {
      ...
      if (groupedItems[key]) {
        ...
        if (!groupedItems[key].orderMonths) {
            groupedItems[key].orderMonths = {} as Record<string, number> // Type assertion here
        }
        
        groupedItems[key].orderMonths[orderMonth] = (groupedItems[key].orderMonths[orderMonth] || 0) + item.orderCount
    } else {
        // Initialize a new group
        groupedItems[key] = { ...item, orderMonths: {} as Record<string, number> } // Type assertion here
        const orderMonth = new Date(item.createdAt).toLocaleString('default', { month: 'long', year: 'numeric' })
        groupedItems[key].orderMonths[orderMonth] = item.orderCount
      }
    })
  
    // After grouping, determine the most frequent month for each group
    Object.values(groupedItems).forEach(item => {
        // Type assertion for the `orderMonths` object
        const mostOrderedMonth = Object.entries(item.orderMonths as Record<string, number>) // Assert the type here
  ...
  {groupedOrderItems.map((item, index) => (
    <tr key={index} className="border-b">
      ...
      <td className="px-2 py-2" style={{ backgroundColor: item.color ?? 'transparent' }}>
</td>
